...
现在的数据、界面和部署链路统一升级为 Schema v5：

- 优先分页读取公开结构化报价接口；接口异常时才启动页面解析回退，减少浏览器渲染成本并显著提高覆盖率。
- 将“渠道新鲜库存”和“原页下单确认”分开显示。
- 增加“严格可购”判定：套餐、价格、明确库存、原商家入口、原页下单动作五项必须同时成立。
- 把成品账号、席位邀请、充值、激活码、网页号、拼车和镜像站拆成独立商品形态，避免跨形态比较最低价。
- 正确解析秒、分钟、小时、天前等相对时间。
...
公开来源的“报价规模”可能超过 2,000 条。系统会并发读取结构化分页并提取可点击商品；只有接口异常时才回退到“加载更多”页面解析。因此以下数字是不同概念：
...
| 独立商家 | 按规范化商家名称去重 |
| 渠道覆盖 | 原商品链接涉及的独立域名 |
| 严格可购 | 五项购买证据同时成立的原页确认记录 |
| 渠道候选 | 套餐、价格、明确库存和原商家入口成立，但下单动作尚未确认 |
...
- 商品类别
- 可购判定：严格可购、渠道候选、当前无货、等待复核
- 商品形态：成品账号、席位、充值、激活码、网页号、拼车、镜像站、辅助服务
- 库存状态
- 证据等级
- 更新新鲜度
- 交付方式
...
- 每条商品都显示可购判定、商品形态、库存、证据等级、新鲜度和可信度。
- 商品详情逐项显示“套餐、价格、库存、入口、下单动作”五项证据，缺一项就不会标记为严格可购。
- 商品详情解释证据来源、原页复核结果、时间、交付方式、域名和历史样本。
- 可选择 2–4 个商品并排比较价格、库存、证据、销量/消耗、交付和价格轨迹。
- 原商品和原店铺链接分开提供。
- 严格长期低价只纳入原页确认且至少已有两个价格样本的记录；渠道低价候选单独展示。
...
| 等级 | 条件 | 能说明什么 | 不能说明什么 |
|---|---|---|---|
| 严格可购 | 套餐识别、匹配价格、明确正库存、原商家入口、原页下单动作五项同页成立，且无售罄冲突 | 核验时原页具备完整购买信号 | 不担保实际付款、发货或售后 |
| 渠道候选 | 公开结构化渠道在 6 小时内返回套餐、价格、明确库存和原商家入口 | 最近的上游库存信号为正，入口格式有效 | 不等同原页已通过下单动作复核 |
...
原页确认最多沿用 12 小时；只要价格变化、库存不再明确为正或商品入口变化，就立即失去“严格可购”资格，等待重新核验。
...
2. 缓存缺失时从当前 Pages 部署恢复最近快照，再以旧快照为历史基线运行新一轮采集。
3. 并发读取 8 个分类的公开 JSON 分页；只有结构化来源失败时才启动 Chromium 页面回退。
...
| 变量 | 默认值 | 用途 |
|---|---:|---|
| `CATALOG_MAX_LOAD_CLICKS` | `60` | 仅页面回退时，每个分类最多展开次数 |
| `CATALOG_MAX_VERIFY` | `320` | 每轮最多原页复核商品数 |
| `CATALOG_VERIFY_CONCURRENCY` | `8` | 原页复核并发 |
| `CATALOG_DISCOVERY_CONCURRENCY` | `2` | 分类发现并发 |
| `CATALOG_API_CONCURRENCY` | `4` | 单分类结构化分页并发 |
| `CATALOG_MAX_API_PAGES` | `80` | 单分类最多读取页数，每页 30 条 |
...
- `channelLowCny`：原页确认或渠道候选样本参与的最低值。
- `strictLowCny`：只有严格原页确认样本参与的最低值。
- `trustedLowCny`：为兼容旧数据保留，在 Schema v5 中等同 `strictLowCny`。
- 严格长期榜使用 `strictLowCny` 且至少需要两个样本；渠道候选榜使用当前渠道价格并明确标注仍需原页核对。
...
```json
{
  ...
  "coverage": {
    ...
    "inStock": 610,
    "strictPurchasable": 3,
    "channelCandidates": 480,
    "finishedAccounts": 620,
    "freshOffers": 690,
    "domains": 57
  },
  "categories": [],
...
- 稳定 ID、类别、商家、商品名
- 当前价格、上一轮价格、观察最低价、渠道最低价、严格最低价
- 库存状态、库存数量、最低起购量
- 原商品链接、原店铺链接、原站域名
- 渠道更新时间、原页复核时间
- 商品形态、可购状态、五项证据清单、证据等级、库存证据来源、可信度和可读理由
- 原页复核状态和原因
- 交付方式和标签
- 首次 / 最近发现时间
- 价格历史、库存历史、7 日库存消耗
...
本地采集（结构化接口正常时无需浏览器；安装 Chromium 后可启用页面故障回退）：
...
scripts/
  adapters/priceai.mjs      公开结构化分页适配器、重试与去重
